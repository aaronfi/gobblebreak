...
var Engine = (function () {
    ...
    function Engine() {
        this.default_level = 5;  // number of moves ahead to search
    }
    ...
    function p4_prepare(state) {
        ...
        var qvalue = P4_VALUES[P4_QUEEN];
        /*used as ballast in various ratios*/
     ...
     * */
    function p4_fen2state(fen, level) {
        if (typeof level === 'undefined') {
            level = this.default_level;
        }

        var state = p4_initialise_state();
        var board = state.board;
        ...
        return p4_findmove(state, level);
    }
    ...
    Engine.prototype.find_best_move = p4_fen2state;
